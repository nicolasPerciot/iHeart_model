...
def dataset_split_index(X, y, test_size = 0.2, fold=None, random_state=42) :
    ...
    index = X.index if isinstance(X, pd.DataFrame) else [i for i in range(len(X))]
    
    if fold == None :
        data_train, data_test, labels_train, labels_test, index_train, index_test = train_test_split(X, y, index, test_size=test_size, random_state=random_state)
